...
| Request Rate (req/s) | `(sum(rate(http_requests_total{namespace="ocr"}[5m])) by (pod) or sum(rate(request_count_total{namespace="ocr"}[5m])) by (pod) or sum(rate(kserve_request_count_total{namespace="ocr"}[5m])) by (pod)) or vector(0)` |
| Inference Latency (P50/P90/P99) | `(histogram_quantile(0.95, sum(rate(http_request_duration_seconds_bucket{namespace="ocr"}[5m])) by (le)) * 1000) or vector(0)` |
| Error Rate (req/s) | `(sum(rate(http_requests_total{namespace="ocr", status!~"2.."}[5m])) or sum(rate(request_count_total{namespace="ocr", code!~"2.."}[5m]))) or vector(0)` |
